import os
import subprocess
import glob
import pandas as pd
from sklearn.metrics import matthews_corrcoef, roc_auc_score
import sys
import shutil

def main(feature_type):
    files = {'physchem': '../model_inputs/cleaned_physchem.csv',
             'maccs': '../model_inputs//cleaned_maccs.csv', 'morgan': '../model_inputs/cleaned_morgan.csv'}


    rf_hyperparams = []
    for trees in range(100, 600, 200):
        for depth in range(10, 50, 10):
            for samples_leaf in range(5, 25, 5):
                rf_hyperparams.append(
                    '{ "min_samples_leaf": ' + str(samples_leaf) + ', "n_estimators": ' + str(trees) + ', "max_depth": ' + str(depth) + ', "random_state": 42, "class_weight": "balanced", "n_jobs": -1}')
    hyperparameters = {
        'rf':  rf_hyperparams,
    }


    task = 'classification'
    model_main = '../models/main_models.py'
    json_config_gen = "../models/json_config_generator.sh"
    dr_name = 'none'
    with open('../model_inputs/sorted_assays.txt', 'r') as assay_file:
        sorted_assays = assay_file.read().splitlines()
    for assay in sorted_assays:

        subfolders = ['androgens', 'estrogens', 'glucocorticoids', 'progestagens', 'steroidal']

        for subfolder in subfolders:
            directory = f'../model_inputs/{subfolder}/'
            response_directory = f'../ToxCastDownloads/binary_responses_and_datasail_input_files/{subfolder}/'
            for content in os.listdir(directory):
                if content != assay:
                    continue
                print(content)
                path_to_CV_folds = f'{directory}/{content}/'
                output_path_to_CV_folds = f'../model_inputs/{subfolder}/{content}/'
                pattern = f'{content}-*_binary_response.csv'

                matching_files = glob.glob(f'{response_directory}/{pattern}')

                if matching_files:
                    response = matching_files[0]
                    print('response found')
                else:
                    continue
                final_models = [False for _ in range(5)]
                for model_name in ['rf']:
                    
                    for fold in range(5):
                        #if not final_models[fold]:
                        #    with open(f'{path_to_CV_folds}/fold{fold}/final_models_{feature_type}_{dr_name}_physchem_samples.txt', 'w') as output:
                        #        output.write('')
                        #    final_models[fold] = True
                        # for hyperparameter combination
                        if not (model_name == 'tabpfn'):
                            best_combi = None
                            best_score = -1
                            for combi in hyperparameters[model_name]:
                                combi_dict = eval(combi)
                                combi_val = 0
                                for ht_fold in range(4):
                                    training_samples = f'{path_to_CV_folds}/fold{fold}/hyperparameter_tuning/fold{ht_fold}/train.txt'
                                    test_samples = f'{path_to_CV_folds}/fold{fold}/hyperparameter_tuning/fold{ht_fold}/test.txt'
                                    if not dr_name == 'none':
                                        features = f'{path_to_CV_folds}/fold{fold}/hyperparameter_tuning/fold{ht_fold}/{feature_type}_feature_names_{dr_name}.txt'
                                    else:
                                        features = f'../model_inputs/all_feature_names_{feature_type}.txt'
                                    if not dr_name == 'pca':
                                        feature_matrix_path = files[feature_type]

                                    else:
                                        feature_matrix_path = f'{path_to_CV_folds}/fold{fold}/hyperparameter_tuning/fold{ht_fold}/{feature_type}_transformed_matrix_pca.csv'

                                    model_specific = '{ "physchem": "", "maccs": "", "morgan": "' + \
                                        feature_matrix_path + \
                                        '", "smiles": "", "morphological": "", "response": "' + response + \
                                        '", "hyperparameters": ' + \
                                        combi + '}'
                                    output_dir = f'../temp/{subfolder}/{content}/fold{fold}/hyperparameter_tuning/fold{ht_fold}/'
                                    os.makedirs(output_dir, exist_ok=True)

                                    config_file_path = f'{output_dir}/{dr_name}_{model_name}_{feature_type}_cleaned_config.json'
                                    analysis_name = f'{model_name}_{feature_type}_{dr_name}_cleaned'
                                    for hp in combi_dict.keys():
                                        analysis_name += f'_{hp}_{combi_dict[hp]}'
                                    subprocess.run(args=['bash', json_config_gen,
                                                        model_name, training_samples, test_samples, features, output_dir, 'deterministic', model_specific, task, analysis_name, config_file_path])
                                    subprocess.run(
                                        ['python3', model_main, config_file_path])
                                    res = pd.read_csv(
                                        f'{output_dir}/{analysis_name}_test_predictions.csv', sep='\t')
                                    mcc = matthews_corrcoef(
                                        y_pred=res['predicted'].values, y_true=res['actual'].values)
                                    combi_val += mcc
                                combi_val /= 4
                                if combi_val > best_score:
                                    best_score = combi_val
                                    best_combi = combi
                                with open(f'{output_path_to_CV_folds}/fold{fold}/hyperparameter_tuning/{model_name}_combis_{feature_type}_{dr_name}.csv', 'a') as ht_val_file:
                                    ht_val_file.write(
                                        f'\n{analysis_name}\t{combi_val}')
                        else:
                            best_combi = hyperparameters['tabpfn'][0]
                        # final test prediction
                        
                        
                        if not dr_name == 'pca':
                            feature_matrix_path = files[feature_type]

                        else:
                            feature_matrix_path = f'{path_to_CV_folds}/fold{fold}/{feature_type}_transformed_matrix_pca.csv'
                        if dr_name != 'none':
                            features = f'{path_to_CV_folds}/fold{fold}/{feature_type}_feature_names_{dr_name}.txt'
                        else:
                            features = f'../model_inputs/all_feature_names_{feature_type}.txt'
                            
                        training_samples = f'{path_to_CV_folds}/fold{fold}/train.txt'
                        test_samples = f'{path_to_CV_folds}/fold{fold}/test.txt'
                        output_dir = f'../temp/{subfolder}/{content}/fold{fold}'
                        output_dir_final_results = f'../model_outputs/{subfolder}/{content}/fold{fold}/'
                        model_specific = '{ "physchem": "", "maccs": "", "morgan": "' + \
                            feature_matrix_path + \
                            '", "smiles": "", "morphological": "", "response": "' + response + \
                            '", "hyperparameters": ' + \
                            best_combi + '}'
                        analysis_name = f'{model_name}_best_combi_{feature_type}_{dr_name}_cleaned'
                        config_file_path = f'{output_dir}/{dr_name}_{model_name}_cleaned_config.json'
                        subprocess.run(args=['bash', json_config_gen,
                                                model_name, training_samples, test_samples, features, output_dir, 'deterministic', model_specific, task, analysis_name, config_file_path])

                        subprocess.run(
                            ['python3', model_main, config_file_path])
                        res = pd.read_csv(
                            f'{output_dir}/{analysis_name}_test_predictions.csv', sep='\t')
                        shutil.copyfile(src = f'{output_dir}/{analysis_name}_test_predictions.csv', dst=f'{output_dir_final_results}/{analysis_name}_test_predictions.csv')
                        mcc = matthews_corrcoef(
                            y_pred=res['predicted'].values, y_true=res['actual'].values)
                        auroc = roc_auc_score(y_true=res['actual'], y_score=res.loc[:,'p(1)'].values)
                        with open(f'{output_dir_final_results}/final_models_{feature_type}_{dr_name}_cleaned.txt', 'a') as output:
                            output.write(
                                f'\n{model_name}\tfold{fold}\t{mcc}\t{auroc}')
                        


if __name__ == '__main__':
    main(sys.argv[1])
