...
def main(feature_type):
    files = {'physchem': '../model_inputs/physchem_properties.csv',
             'maccs': '../model_inputs//maccs.csv', 'morgan': '../model_inputs/morgan_physchem_samples.csv'}
    ...
    for assay in sorted_assays:
        ...
        for subfolder in subfolders:
            ...
            response_directory = f'../ToxCastDownloads/binary_responses_and_datasail_input_files/{subfolder}/'
            for content in os.listdir(directory):
                if content != assay:
                    continue
                print(content)
                path_to_CV_folds = f'{directory}/{content}/'
                output_path_to_CV_folds = f'../model_inputs/{subfolder}/{content}/'
                pattern = f'{content}-*_binary_response.csv'

                matching_files = glob.glob(f'{response_directory}/{pattern}')
                ...
                for model_name in ['rf']:
                    ...
                    for fold in range(5):
                        ...
                        #    with open(f'{path_to_CV_folds}/fold{fold}/final_models_{feature_type}_{dr_name}_physchem_samples.txt', 'w') as output:
                        #        output.write('')
                            ...
                            for combi in hyperparameters[model_name]:
                                ...
                                for ht_fold in range(4):
                                    ...
                                    if not dr_name == 'none':
                                        ...
                                        features = f'../model_inputs/all_feature_names_{feature_type}_without_issues.txt'
                                    if not dr_name == 'pca':
                                    ...
                                    config_file_path = f'{output_dir}/{dr_name}_{model_name}_removed_problematic_descriptors_config.json'
                                    analysis_name = f'{model_name}_{feature_type}_{dr_name}_removed_problematic_descriptors'
                                ...
                                if combi_val > best_score:
                                    ...
                                    best_combi = combi
                                with open(f'{output_path_to_CV_folds}/fold{fold}/hyperparameter_tuning/{model_name}_combis_{feature_type}_{dr_name}.csv', 'a') as ht_val_file:
                                    ht_val_file.write(
                        ...
                        output_dir_final_results = f'../model_outputs/{subfolder}/{content}/fold{fold}/'
                        model_specific = '{ "physchem": "", "maccs": "", "morgan": "' + \
                            feature_matrix_path + \
                            '", "smiles": "", "morphological": "", "response": "' + response + \
                            '", "hyperparameters": ' + \
                            best_combi + '}'
                        analysis_name = f'{model_name}_best_combi_{feature_type}_{dr_name}_removed_problematic_descriptors'
                        config_file_path = f'{output_dir}/{dr_name}_{model_name}_removed_problematic_descriptors_config.json'
                        subprocess.run(args=['bash', json_config_gen,
                                                model_name, training_samples, test_samples, features, output_dir, 'deterministic', model_specific, task, analysis_name, config_file_path])
                        ...
                        res = pd.read_csv(
                            f'{output_dir}/{analysis_name}_test_predictions.csv', sep='\t')
                        shutil.copyfile(src = f'{output_dir}/{analysis_name}_test_predictions.csv', dst=f'{output_dir_final_results}/{analysis_name}_test_predictions.csv')
                        mcc = matthews_corrcoef(
                            y_pred=res['predicted'].values, y_true=res['actual'].values)
                        auroc = roc_auc_score(y_true=res['actual'], y_score=res.loc[:,'p(1)'].values)
                        with open(f'{output_dir_final_results}/final_models_{feature_type}_{dr_name}_removed_problematic_descriptors.txt', 'a') as output:
                            output.write(
