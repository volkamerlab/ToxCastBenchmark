    ...
    for dr_name in ['MI', 'mrmr', 'variance', 'pca', 'none']:
        ...
        for subfolder in subfolders:
            ...
            for content in os.listdir(directory):
                ...
                for model_name in ['tabpfn', 'cat_boost', 'rf', 'mlp', 'svm']:
                    
                    for fold in range(5):
                        if not final_models[fold]:
                            with open(f'{path_to_CV_folds}/fold{fold}/final_models_{feature_type}_{dr_name}.txt', 'w') as output:
                                output.write('')
                        ...
                        if not (model_name == 'tabpfn'):
                            ...
                            for combi in hyperparameters[model_name]:
                                ...
                                if combi_val > best_score:
                                    ...
                                    best_combi = combi
                                with open(f'{path_to_CV_folds}/fold{fold}/hyperparameter_tuning/{model_name}_combis_{feature_type}_{dr_name}.csv', 'a') as ht_val_file:
                                    ht_val_file.write(
                        ...
                        with open(f'{output_dir_final_results}/final_models_{feature_type}_{dr_name}.txt', 'a') as output:
                            output.write(
