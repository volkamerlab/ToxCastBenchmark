...
class Support_Vector_Machine(Base_Model):
    ...
    def __init__(self, data_json_dict):
        ...
        if data_json_dict['task'] == 'classification':
            ...
            self.model = SVR()
        if 'probability' in self.hyperparameters.keys():
            self.hyperparameters['probability'] = bool(
                self.hyperparameters['probability'])
    ...
    def fit(self):
        ...
        X_train_transformed = self.scaler.fit_transform(X_train, y_train)
