...
class MLP(Base_Model):
    ...
    def __init__(self, data_json_dict):
        ...
        if 'hidden_layer_sizes' in self.hyperparameters.keys():
            if type(self.hyperparameters['hidden_layer_sizes']) == str:
                self.hyperparameters['hidden_layer_sizes'] = eval(
                    self.hyperparameters['hidden_layer_sizes'])
        if 'early_stopping' in self.hyperparameters.keys():
            self.hyperparameters['early_stopping'] = bool(
    ...
    def fit(self):
        ...
        X_train_scaled = self.scaler.fit_transform(X_train, y_train)
