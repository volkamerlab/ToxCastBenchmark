...
def plot_performance_versus_assay_size(plot_df, outdir, col='representation'):
...
def plot_performance_vs_imbalance(plot_df, outdir, col='representation'):
    plt.figure(figsize=(7, 6))
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{outdir}/performance_vs_imbalance_{col}.png')


def boxplot_best_performances(df_avg, outdir, metric='mcc_avg', ylab="Best MCC per assay (Fisher-averaged)", col='representation'):
    print(df_avg)
    best_per_assay = (
        df_avg.loc[
    ...
    plt.tight_layout()
    plt.savefig(
        f'{outdir}/boxplot_with_best_performances_{metric}_{col}.png', dpi=600)
...
def plot_heatmap(df, outpath, metric='mcc', col='representation'):
    ...
    plt.title("Top-k assay wins per representation-DR method-model combination",
              fontsize=16, pad=30)
    plt.xlabel("")
    ...
    if metric == 'auroc':
        boxplot_best_performances(
            df_avg, outpath, metric='auroc', ylab='Best AUROC per assay')
    elif metric == 'mcc':
        boxplot_best_performances(df_avg, outpath, col=col)
        ...
        plot_performance_vs_imbalance(plot_df, outpath, col=col)

        size_dict = compute_assay_size(df)

        plot_df = create_plot_df(best_per_assay, size_dict)

        plot_performance_versus_assay_size(plot_df, outpath, col=col)
...
def main(args):
    ...
    plot_heatmap(final_results, out_dir, args.metric, col='representation')
    plot_heatmap(final_results, out_dir, args.metric, col='model')
    plot_heatmap(final_results, out_dir, args.metric, col='dr_method')
